...
export const Formulario = ( { aoSubmeter } : FormularioProps ) => {
    ...
    return(
        <form onSubmit={adicionarFilme}>
            <input type="text" placeholder="Insira o nome do filme aqui" onChange={evento => setFilme({...filme, nome: evento.target.value})} required/>
            
            <input type="text" placeholder="Digite o ano de lançamento" onChange={evento => setFilme({...filme, anoDeLancamento: evento.target.value})} required/>
